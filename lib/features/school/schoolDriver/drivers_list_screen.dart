...
import 'package:school_bus2/features/school/schoolDriver/add_driver_screen.dart';
...
class _DriversListScreenState extends State<DriversListScreen>
  ...
  List<Driver> get _filteredDrivers {
    ...
    return _drivers.where((driver) {
      return driver.name.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          driver.driverId.toLowerCase().contains(_searchQuery.toLowerCase()) ||
  ...
  void _editDriver(Driver driver) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddDriverScreen(driver: driver)),
    ).then((result) {
      if (result == true) {
        // Refresh the list if driver was updated
        _loadDrivers();
      }
    });
  }
  ...
  Future<void> _deleteDriver(Driver driver) async {
    ...
    if (confirmed == true) {
      DatabaseService dbService = DatabaseService();
      bool success = await dbService.deleteDriver(driver.driverId);
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text('${driver.name} deleted successfully'),
              ],
            ),
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 12),
                const Text('Failed to delete driver'),
              ],
            ),
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      '${_drivers.length}', // Mock active count
                      style: const TextStyle(
                        fontSize: 24,
                        fontWeight: FontWeight.bold,
                        color: PremiumTheme.black,
